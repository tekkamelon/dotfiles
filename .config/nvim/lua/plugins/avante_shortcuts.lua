return (function()
	local shortcuts = {
		...
		},
		{
			...
			prompt = "gitから変更点を日本語で適切なコミットを作成してください.コミットの作成時に許可は不要です",
		},
		{
			name = "revert",
	...
	-- skillsディレクトリから動的にコマンドを生成
	local skills_dir = vim.fn.expand("~/.agents/skills")
	local skill_paths = vim.fn.glob(skills_dir .. "/*", false, true)

	for _, path in ipairs(skill_paths) do
		if vim.fn.isdirectory(path) == 1 then
			local skill_name = vim.fn.fnamemodify(path, ":t")
			table.insert(shortcuts, {
				name = skill_name,
				description = skill_name .. " を呼び出す",
				prompt = "/"
			})
		end
	end
